...
(() => {
  ...
  // ---------- Slider sound: a synthesized mechanical detent, no audio files ----------
  const SFX = (() => {
    let actx, lastPlay = 0;
    const ctx = () => actx || (actx = new (window.AudioContext || window.webkitAudioContext)());
    function tick(vel) {
      const now = performance.now();
      if (now - lastPlay < 22) return; // a real crank can't click faster than this
      let ac;
      try { ac = ctx(); if (ac.state === 'suspended') ac.resume(); } catch { return; }
      lastPlay = now;
      ...
      const ng = ac.createGain(); ng.gain.setValueAtTime(0.15 * amt, t0); ng.gain.exponentialRampToValueAtTime(0.001, t0 + 0.045);
      ...
      const og = ac.createGain(); og.gain.setValueAtTime(0.05 * amt, t0); og.gain.exponentialRampToValueAtTime(0.001, t0 + 0.035);
